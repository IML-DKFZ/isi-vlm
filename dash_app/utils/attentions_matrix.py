...
def remove_default_prompt(
    attention_scores: Tuple[Tuple[torch.Tensor]], default_prompt_length: int
...
def get_out_attn_matrix(attention_scores):
    ...
    llm_out_attn_matrix = heterogenous_stack(
...
class MultiModalAttention:
    ...
    def __call__(self, attention_scores, prompt, description, question):
        ...
        scores = self.multimodal_attention_aggregation(
            llm_out_attn_matrix, prompt, description, question
        )
        return scores
    ...
    def multimodal_attention_aggregation(
        ...
        attn_to_description = llm_out_attn_matrix[
            :-1, P["description_start"] : P["description_end"]
        ]  #:-1 is to exclude the <eos> or </s> token
